# Trace Personal App

Desktop MVP scaffold for the personal market decision cockpit.

## Stack

- Vite + React + TypeScript for the UI.
- FastAPI + uvicorn for the warm local Trace engine API.
- Python for the deterministic Trace engine layer.
- Desktop shell is deferred; Electron/Tauri can wrap the web app later.

This is intentionally web-first. It avoids blocking the MVP on Rust/Tauri or Electron packaging
while preserving a future migration path to a desktop wrapper.

## Commands

Run from `app/`:

```powershell
pip install -r requirements.txt
cmd /c npm install
cmd /c npm run api:dev
cmd /c npm run dev
```

In another terminal:

```powershell
cmd /c npm install
cmd /c npm run engine:demo
cmd /c npm run build
```

PowerShell may block `npm.ps1` on this machine, so prefer `cmd /c npm ...`.

Current machine note: the earlier Electron dependency install failed with `ENOSPC: no space left on
device`. Electron is no longer a default dependency, so the npm install should be much smaller. The
Python engine demo does not depend on npm and already runs.

## Engine Boundary

The UI does not invent quantitative outputs. It calls:

```text
POST http://127.0.0.1:8000/api/run-personal-advice
```

The FastAPI server imports:

- `engine/trace_engine.py`
- `engine/trace_structured.py`
- `engine/trace_bridge.py`
- `engine/schema_adapter.py`
- `engine/run_demo.py`

The app-facing contract keeps these axes separate:

- `candidate_support`
- `candidate_coverage`
- `candidate_credibility`
- `candidate_status`
- `claim_resolution`
- `trace_log`
- `personal_fit`

Do not collapse them into a single trust score.

The UI also reads:

```text
GET http://127.0.0.1:8000/api/factor-tree
GET http://127.0.0.1:8000/api/personal-profiles
```

`/api/factor-tree` serves the compact mixed-source factor fixture generated by:

```powershell
python scripts\build_nl_housing_app_research_fixture.py
```

Every user-facing figure should trace to engine output, `fixtures/nl_housing/*`, or a fetched
source URL. The hardcoded `engine/run_demo.py` case is not allowed for user-facing numbers.
Only nested source objects directly extracted from raw Cala responses may carry
`source_origin: "cala"` / `source:cala`; web/tool evidence uses `source:web`, and
factor nodes use `derived_from_source_origins`.

Research evidence is tagged with `probability_pct`. Already happened / already-in-force
items are `100`; model estimates start at `80`; analysis/news claims at `60`;
forecasts at `55`; policy targets/proposals at `50`; risk scenarios at `35`; and
rumor/unverified claims at `15`, with small source/framing adjustments in the fixture
builder. Keep `evidence_status` as audit context, but the UI should display the percent.
